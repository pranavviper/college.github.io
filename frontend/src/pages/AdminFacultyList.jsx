...
const AdminFacultyList = () => {
    ...
    const [formData, setFormData] = useState({
        name: '', email: '', password: 'Changeme@123', role: 'faculty', department: '', registerNumber: ''
    });
    ...
    const handleAddUser = async (e) => {
        ...
        try {
            const config = { headers: { Authorization: `Bearer ${user.token}` } };
            const dataToSend = { ...formData, role: 'faculty' };
            await axios.post(`${API_URL}/admin/users`, dataToSend, config);
            setIsAddUserModalOpen(false);
            setFormData({ name: '', email: '', password: 'Changeme@123', role: 'faculty', department: '', registerNumber: '' });
            fetchUsers();
        } catch (err) {
    ...
    const handleEditUser = async (e) => {
        ...
        try {
            ...
            setEditingUser(null);
            setFormData({ name: '', email: '', password: '', role: 'faculty', department: '', registerNumber: '' });
            fetchUsers();
        } catch (err) {
        ...
        <div className="max-w-7xl mx-auto animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">View and manage all faculty members.</p>
                </div>
            </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            ...
                            {filteredUsers.length === 0 && (
                                <tr>
                                    <td colSpan="4" className="p-8 text-center text-slate-400">No faculty members found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                    ...
                    <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                        ...
                        <form onSubmit={handleAddUser} className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={e => {
                                        ...
                                        // Simple email generation: firstname.lastname@ or just name@? 
                                        // User said "username". Let's assume standard lowercased name without spaces.
                                        const emailPrefix = name.toLowerCase().replace(/\s+/g, '');
                                        setFormData({
                                            ...formData,
                                            name: name,
                                            email: emailPrefix ? `${emailPrefix}@rajalakshmi.edu.in` : ''
                                        });
                                    }}
                                />
                            </div>
                            ...
                            <div className="pt-4 flex gap-3">
                                ...
                                <button type="submit" className="btn btn-primary flex-1">Create Faculty</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            {isEditUserModalOpen && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold text-slate-800">Edit Faculty</h2>
                        ...
                        <form onSubmit={handleEditUser} className="space-y-4">
                            ...
                            <div className="pt-4 flex gap-3">
                                ...
                                <button type="submit" className="btn btn-primary flex-1">Update Faculty</button>
                            </div>
                        </form>
                    </div>
                </div>
